...
/**
 * @title DeployTopOfBlockAuction
 * @notice Deploys TopOfBlockAuction deterministically to any chain with a Uniswap v4 `PoolManager`.
 *
 ...
 */
contract DeployTopOfBlockAuction is Script {
    ...
    function run() external {
        ...
        bytes memory creationCode = type(TopOfBlockAuctionHook).creationCode;
        ...
        console2.log("TopOfBlockAuction ", predicted);
        ...
        vm.startBroadcast();
        TopOfBlockAuctionHook hook = new TopOfBlockAuctionHook{salt: salt}(manager);
        vm.stopBroadcast();
        ...
        require(
            keccak256(bytes(IHookMetadata(address(hook)).hookName())) == keccak256(bytes("TopOfBlockAuction")),
